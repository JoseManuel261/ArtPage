...
create table if not exists public.tableros (
  ...
  modo text not null default 'collage' check (modo in ('collage', 'album', 'timeline', 'presentacion')),
  tema_visual text not null default 'neon' check (tema_visual in ('neon', 'scrapbook', 'pastel')),
...
alter table public.tableros add column if not exists tema_visual text not null default 'neon';
alter table public.tableros add column if not exists fecha_revelacion timestamptz;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'tableros_modo_check') then
    alter table public.tableros
      add constraint tableros_modo_check check (modo in ('collage', 'album', 'timeline', 'presentacion'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'tableros_tema_check') then
    alter table public.tableros
      add constraint tableros_tema_check check (tema_visual in ('neon', 'scrapbook', 'pastel'));
  end if;
end $$;
