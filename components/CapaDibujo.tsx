...
interface CapaDibujoProps {
  tablero: Tablero;
  /** Modo overlay: se usa como capa flotante encima de otro lienzo (ej. el Collage),
   * con fondo transparente y boton de cerrar. Modo standalone (false): pantalla completa,
   * fondo blanco tipo hoja de papel, sin boton de cerrar. */
  overlay?: boolean;
  /** Solo aplica en modo overlay: si esta capa esta activa recibiendo el puntero. */
  activo?: boolean;
  onCerrar?: () => void;
}
...
/**
 * Motor de dibujo compartido: se usa tanto en el modo "Dibujo"
 * standalone (tablero dedicado, hoja en blanco) como superpuesto
 * dentro del modo "Collage" (capa de pintura encima de las fotos).
 * Un mismo motor, dos formas de mostrarlo — no hay dos
 * implementaciones separadas del dibujo en la app.
 *
 * Pinceles con algoritmos distintos: Lapiz (textura granulada),
 * Marcador (plano, se oscurece al superponerse), Pincel (grosor
 * variable segun velocidad, textura de cerdas), Acuarela (manchas
 * translucidas que sangran entre si), Aerografo (nube de puntos
 * dispersos), Borrador (borra pixeles de verdad).
 *
 * Pointer Events unificados: funciona igual con mouse, dedo, o lapiz
 * optico.
 */
export default function CapaDibujo({ tablero, overlay = false, activo = true, onCerrar }: CapaDibujoProps) {
  const tema = useTema();
  ...
  const fondoBlanco = !overlay; // hoja de papel en standalone; transparente en overlay

  function pintarFondo(ctx: CanvasRenderingContext2D, width: number, height: number) {
    if (fondoBlanco) {
  ...
  const ajustarTamanoCanvas = useCallback(() => {
    ...
    const anterior = canvas.width > 0 ? canvas.toDataURL() : null;
  ...
  return (
    <div
      className={raizClase}
      style={{ backgroundColor: overlay ? "transparent" : tema.fondo, zIndex: overlay ? 9999 : undefined }}
    >
      <div
        ref={contenedorRef}
        className={`relative flex-1 touch-none overflow-hidden ${overlay ? "pointer-events-none" : ""}`}
      >
        <canvas
          ref={canvasRef}
          className={`absolute inset-0 h-full w-full touch-none ${overlay && !activo ? "pointer-events-none" : "pointer-events-auto"}`}
      ...
      {/* Barra de estado: guardado (solo standalone; en overlay va en la barra de herramientas) */}
      {!overlay && (
        <div
          className="pointer-events-none absolute right-3 top-3 z-30 flex items-center gap-1 px-2 py-1 text-[10px]"
      ...
      {/* Barra de herramientas: siempre visible cuando esta activa */}
      {(!overlay || activo) && (
        <div
          className="pointer-events-auto relative z-30 flex flex-wrap items-center gap-2 border-t p-2 sm:p-3"
        ...
        >
          ...
          <div className="flex gap-1">
            {PINCELES.map((p) => {
              ...
              const activoPincel = pincel === p.valor;
              return (
                <button
                  key={p.valor}
                  ...
                  style={{
                    backgroundColor: activoPincel ? tema.acento : "transparent",
                    color: activoPincel ? tema.fondo : tema.textoSuave,
                    borderRadius: tema.bordeRadio / 2,
                    border: `1px solid ${activoPincel ? tema.acento : `${tema.textoSuave}33`}`,
                  }}
                >
          ...
          >
            <span
              ...
              style={{
                ...
                width: Math.min(28, Math.max(10, tamano)),
                height: Math.min(28, Math.max(10, tamano)),
              }}
            />
          </button>
